...
from db.models import get_product_id_by_name, get_product_by_id, get_cart_items, add_to_cart, update_cart_item_quantity, load_cart_for_user, save_to_cart, clear_cart
...
class CartController:
    ...
    def update_cart_ui(self):
        ...
        self.ui.tableWidget_cart.setRowCount(0)  # Очищаем таблицу

        for row, item in enumerate(self.cart_items):
            self.ui.tableWidget_cart.insertRow(row)
            id_item = QTableWidgetItem(str(item["id_product"]))
            id_item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)    # Делаем ячейку нередактируемой
            self.ui.tableWidget_cart.setItem(row, 0, id_item)

            name_item = QTableWidgetItem(item["model_name"])
            name_item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)  # Делаем ячейку нередактируемой
            self.ui.tableWidget_cart.setItem(row, 1, name_item)

            price_item = QTableWidgetItem(f"{item['price']:.2f}")
            price_item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)  # Делаем ячейку нередактируемой
            self.ui.tableWidget_cart.setItem(row, 2, price_item)

            # Создаём редактируемую ячейку для количества
            quantity_item = QTableWidgetItem(str(item["quantity"]))
            quantity_item.setFlags(quantity_item.flags() | Qt.ItemIsEditable)  # Делаем ячейку редактируемой
            self.ui.tableWidget_cart.setItem(row, 3, quantity_item)

            # Итоговая стоимость
            total_price = item["quantity"] * item["price"]
            total_price_item = QTableWidgetItem(f"{total_price:.2f}")
            total_price_item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)  # Делаем ячейку нередактируемой
    ...
    def on_quantity_changed(self, item):
        ...
        # Проверяем, изменена ли ячейка количества (3-й столбец)
        if column == 3:
            try:
                ...
                product_id = int(self.ui.tableWidget_cart.item(row, 0).text())  # ID товара
                price = float(self.ui.tableWidget_cart.item(row, 2).text())  # Цена товара
                ...
                previous_quantity = self.ui.tableWidget_cart.item(item.row(), 3).text()  # Старое значение
                item.setText(previous_quantity)  # Восстанавливаем старое значение
    ...
    def load_product_data(self, product_id):
        ...
        if product:
            ...
            self.ui.quantity_input.setText("1")  # По умолчанию 1
        else:
            self.show_message("Товар не найден!")


    def on_add_to_cart_button_click(self):
        """Обработчик кнопки 'Добавить в корзину' на странице товара."""
        if not self.user_id:
            self.show_message("Пользователь не авторизован!")
            return
    ...
    def show_product_page(self, product_id):
        ...
        self.ui.stackedWidget.setCurrentIndex(6)  # Индекс страницы товара
    ...
    def on_msg_closed():
        self.ui.stackedWidget.setCurrentIndex(4)  # Переключаем на корзину

        msg.finished.connect(on_msg_closed)
        msg.exec_()  # Ожидаем закрытия сообщения
